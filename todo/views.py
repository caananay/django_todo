...
class TodoView(APIView):
    ...
    def post(self, request):
        ...
        if not serializer.is_valid():
            ...
            Todo.objects.create(user=user, title=data["title"], description=data["description"],
                                status=data["status"])
    ...
    def put(self, request, pk):
        ...
        # Check to see if the data in the `request` is valid.
        # If the cannot be deserialized into a to do
        # object then a bad request response will be returned containing the error.
